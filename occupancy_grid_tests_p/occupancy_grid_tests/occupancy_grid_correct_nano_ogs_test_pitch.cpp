//  g++ occupancy_grid_correct_nano_ogs_test_pitch.cpp -o occupancy_grid `pkg-config --cflags --libs opencv4`
...
int main(){
    ...
    sprintf(imread_buffer, "../correct_nano_ogs/imgs/originals_output%03d.png", img_number);
    ...
    for(int i = 0; i < grid_dims[0]; i++){
        ...
        for(int j = 0; j < grid_dims[1]; j++){
            ...
            if (pixel_w >= cols || pixel_w < 0 || pixel_h >= rows || pixel_h < 0){
                ...
                if(i >= 70 && i <= 75 || i >= 90 && i<=95){
                    grid_image.at<cv::Vec3b>((grid_dims[0]-1) - i, j) = cv::Vec3b(0,0,255);
                }
